...
The Goggles application SHALL receive DMA-BUF frames and display them via SDL. This is a temporary implementation for layer validation; zero-copy import will be added in a future iteration.

#### Scenario: DMA-BUF mmap and display
- **GIVEN** the app receives a DMA-BUF fd via socket
- **WHEN** the fd and metadata are valid
- **THEN** the app SHALL mmap the fd for CPU access
- **AND** use `DMA_BUF_IOCTL_SYNC` before reading
- **AND** update an SDL_Texture with the pixel data
- **AND** render the texture to the window

#### Scenario: No capture source
- **GIVEN** the Goggles app is running
- **WHEN** no layer has connected to the socket
- **THEN** the app SHALL display a black/clear screen
