...
constexpr std::array<WindowConfig, WINDOW_COUNT> WINDOW_CONFIGS = {{
    {"Surface Test 1 (X11)", 60, 0, 0},
    {"Surface Test 2 (X11)", 0, 60, 0},
    {"Surface Test 3 (X11)", 0, 0, 60},
}};
...
static auto init_vulkan() -> VulkanInstance {
    ...
    app_info.pApplicationName = "Goggles Surface Selector Test (X11)";
    ...
    if (result != VK_SUCCESS) {
        fprintf(stderr, "[goggles_manual_surface_selector_x11] vkCreateInstance failed: %d\n",
                result);
        std::exit(1);
    }
...
static auto init_sdl_windows() -> MultiWindowContext {
    ...
    if (!SDL_Init(SDL_INIT_VIDEO)) {
        fprintf(stderr, "[goggles_manual_surface_selector_x11] SDL_Init failed: %s\n",
                SDL_GetError());
        std::exit(1);
    }
    ...
    for (size_t i = 0; i < WINDOW_COUNT; ++i) {
        ...
        if (!ctx.windows[i]) {
            fprintf(stderr,
                    "[goggles_manual_surface_selector_x11] SDL_CreateWindow failed for window %zu: "
                    "%s\n",
                    i + 1, SDL_GetError());
            std::exit(1);
        }
        ...
        if (!ctx.renderers[i]) {
            fprintf(stderr,
                    "[goggles_manual_surface_selector_x11] SDL_CreateRenderer failed for window "
                    "%zu: %s\n",
                    i + 1, SDL_GetError());
            std::exit(1);
        }
...
static void print_instructions() {
    printf("===========================================\n");
    printf("Goggles Surface Selector Test (X11)\n");
    printf("Tests multi-surface input routing\n");
    printf("===========================================\n");
    printf("This test creates 3 XWayland surfaces.\n");
...
auto main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) -> int {
    fprintf(stderr, "[goggles_manual_surface_selector_x11] Starting\n");

    const char* display = getenv("DISPLAY");
    if (!display || display[0] == '\0') {
        fprintf(stderr, "[goggles_manual_surface_selector_x11] DISPLAY not set.\n");
        fprintf(stderr, "Run this test via: pixi run start debug <this_executable>\n");
        return 1;
    }
    fprintf(stderr, "[goggles_manual_surface_selector_x11] DISPLAY='%s'\n", display);

    auto vk = init_vulkan();
    fprintf(stderr, "[goggles_manual_surface_selector_x11] Vulkan instance created\n");

    auto ctx = init_sdl_windows();
    fprintf(stderr, "[goggles_manual_surface_selector_x11] SDL windows created\n");

    run_event_loop(ctx);

    fprintf(stderr, "[goggles_manual_surface_selector_x11] Exiting\n");
    return 0;
}
