...
layer_early_init()
{
	...
	socklen_t addr_len = static_cast<socklen_t>(
	    offsetof(sockaddr_un, sun_path) + goggles::capture::CAPTURE_SOCKET_PATH_LEN);
	...
	if (response.type != goggles::capture::CaptureMessageType::input_display_ready) {
		LAYER_DEBUG("layer_early_init: Unexpected message type %u", static_cast<uint32_t>(response.type));
		return;
	}
