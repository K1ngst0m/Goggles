...
auto InputForwarder::forward_mouse_motion(const SDL_MouseMotionEvent& event) -> Result<void> {
    ...
    // This causes mouse position to be incorrect when viewer and target windows have different sizes
    // Needs: scale factor calculation, window dimension tracking, coordinate space transformation
    // See OpenSpec for coordinate mapping task
